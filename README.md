...
6.  「環境設定」として利用するチャンクまたは設定ファイルを作成します。
    * チャンクとして作成する場合：任意の名前でチャンクを作成し、/assets/snippets/cfFormMailer/forms/sample/ フォルダ内の config.with_comment.ini の内容を参考に設定を記述します。
    * ファイルとして作成する場合：config.with_comment.ini をコピーして任意の場所に配置します。
    * スニペットコールで、チャンク名または設定ファイルのパス（`@FILE:相対パス`形式）を指定します。
...
環境設定（チャンクまたは設定ファイル）には、以下の設定項目が用意されています。
...
6.  「自動返信メール（モバイル用）」テンプレートの作成　※PCメール宛と携帯メール宛でテンプレートを変えたい場合
    ...
    ここで作成したチャンクの名称を、環境設定の tmpl_mail_reply_mobile 項目に記述します。  
...
※バージョン1.0以降、動作検証は MODX Evolution 1.x 日本版にて行っています。また文字コードは UTF-8 に限定しております。
