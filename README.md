...
6.  「環境設定」として利用する設定ファイルを作成します。
    ...
    * スニペットタグで、設定ファイルのパスまたはチャンク名を指定します。
...
上記インストール手順6で作成する設定ファイルには、以下の設定項目が用意されています。
...
config パラメータは必須です。設定ファイルのパスまたはチャンク名を指定します。
...
設定ファイル内の **auto_reply** を 1 、**reply_to** にメールアドレスとして使用するフィールド名を指定することで、"管理者"宛のメールのほかにフォーム送信者に対して自動返信を行うことができます。
...
設定ファイル内で CAPTCHA 使用を宣言し（**vericode = 1**）、以下の例のような、src 属性に **[+verimageurl+]** プレースホルダを指定した`<img>`タグと、検証項目に **vericode** を指定した`<input>` タグを、入力画面テンプレート内に作成します。
...
使用するテンプレートをチャンクとして作成し、そのチャンク名を設定ファイルの「tmpl_mail_reply_mobile」項目に指定します。
...
[注意]
"SAFE MODE Restriction in effect"というエラーが発生する場合は、設定ファイルのupload_tmp_pathを1にしてみてください。
